...
namespace REDFS_ClusterMode
{
    ...
    public class fingerprintBACKUP : Item
    {
        ...
        public Boolean Equals(fingerprintBACKUP other)
        {
            if (this.inode != other.inode || this.fbn != other.fbn)
            {
                return false;
            }
    ...
    public class fingerprintDMSG : Item
    {
        ...
        public long sourcedbn, destinationdbn;
        public byte[] fp = new byte[16];
        ...
        public Boolean Equals(fingerprintDMSG other)
        {
            if (this.fsid != other.fsid || this.inode != other.inode || this.fbn != other.fbn ||
                this.sourcedbn != other.sourcedbn || this.destinationdbn != other.destinationdbn)
            {
                return false;
            }
            for (int i=0;i<fp.Length;i++)
        ...
        int Item.get_size() { return 44; }
        Item Item.create_new_obj() { return new fingerprintDMSG(this.stype); }
        ...
        void Item.get_bytes(byte[] buffer, int offset)
        {
            ...
            OPS.set_long(buffer, offset + 12, sourcedbn);
            OPS.set_long(buffer, offset + 20, destinationdbn);
            for (int i = (offset + 28), k = 0; i < (offset + 44); i++, k++)
            {
                buffer[i] = fp[k];
            }
        }
        ...
        void Item.parse_bytes(byte[] buffer, int offset)
        {
            ...
            sourcedbn = OPS.get_long(buffer, offset + 12);
            destinationdbn = OPS.get_long(buffer, offset + 20);
            for (int i = (offset + 28), k = 0; i < (offset + 44); i++, k++)
            {
                fp[k] = buffer[i];
            }
    ...
    public class fingerprintFPDB : Item
    {
        ...
        public Boolean Equals(fingerprintFPDB other)
        {
            if (this.dbn != other.dbn)
            {
                return false;
            }
        ...
        int Item.get_size() { return 24; }
        Item Item.create_new_obj() { return new fingerprintFPDB(this.stype); }
        void Item.set_cookie(int c) { _cookie = c; }
        int Item.get_cookie() { return _cookie; }

        void Item.get_bytes(byte[] buffer, int offset)
        {
            OPS.set_long(buffer, offset + 0, dbn);
            for (int i = (offset + 8), k = 0; i < (offset + 24); i++, k++)
            {
                buffer[i] = fp[k];
            }
        ...
        void Item.parse_bytes(byte[] buffer, int offset)
        {
            dbn = OPS.get_long(buffer, offset + 0);
            for (int i = (offset + 8), k = 0; i < (offset + 24); i++, k++)
            {
                fp[k] = buffer[i];
            }
    ...
    public class fingerprintCLOG : Item
    {
        ...
        public Boolean Equals(fingerprintCLOG other)
        {
            if (this.fsid != other.fsid || this.inode != other.inode || this.fbn != other.fbn ||
                this.dbn != other.dbn || this.cnt != other.cnt)
            {
                return false;
            }
        ...
        void Item.get_bytes(byte[] buffer, int offset)
        {
            ...
            OPS.set_int(buffer, offset + 4, inode);
            OPS.set_int(buffer, offset + 8, fbn);
            OPS.set_long(buffer, offset + 12, dbn);
            OPS.set_int(buffer, offset + 20, cnt);

            for (int i = (offset + 24), k = 0; i < (offset + 40); i++, k++)
            {
                buffer[i] = fp[k];
            }
        }
        ...
        void Item.parse_bytes(byte[] buffer, int offset)
        {
            ...
            inode = OPS.get_int(buffer, offset + 4);
            fbn = OPS.get_int(buffer, offset + 8);
            dbn = OPS.get_long(buffer, offset + 12);
            cnt = OPS.get_int(buffer, offset + 20);
            for (int i = (offset + 24), k = 0; i < (offset + 40); i++, k++)
            {
                fp[k] = buffer[i];
            }
